...
start "ProxyYARP - ControlPlane" cmd /k "set ASPNETCORE_URLS=http://localhost:8080&& set Management__Enabled=true&& set Management__GroupId=default&& set Management__NodeName=ControlPlane&& set DB_TYPE=sqlite&& set DB_CONNECTION=Data Source=data\test.db&& dotnet run --no-build --project src\ProxyYARP\ProxyYARP.csproj"
...
start "ProxyYARP - Worker 1" cmd /k "set ASPNETCORE_URLS=http://localhost:8081&& set Management__Enabled=false&& set Management__GroupId=default&& set Management__NodeName=Worker-1&& set DB_TYPE=sqlite&& set DB_CONNECTION=Data Source=data\test.db&& dotnet run --no-build --project src\ProxyYARP\ProxyYARP.csproj"

echo [4/4] Starting ProxyYARP Worker Node 2 (Port: 8082)...
start "ProxyYARP - Worker 2" cmd /k "set ASPNETCORE_URLS=http://localhost:8082&& set Management__Enabled=false&& set Management__GroupId=default&& set Management__NodeName=Worker-2&& set DB_TYPE=sqlite&& set DB_CONNECTION=Data Source=data\test.db&& dotnet run --no-build --project src\ProxyYARP\ProxyYARP.csproj"
