...
{
    public DestinationRepository(System.Data.IDbConnection? connection = null) : base(connection) { }
    ...
    public List<ProxyDestinationEntity> GetByClusterId(string clusterId)
    {
        return WithConnection(c => c.Query<ProxyDestinationEntity>(
            "SELECT * FROM ProxyDestinations WHERE ClusterId = @ClusterId AND IsEnabled = 1",
